...
def e_servico(f, R, RES, prof):
    """🔴 S42 — esta PANCADA é um SERVIÇO?  (Vasco, 13 jul)

    "NÃO HÁ SERVIÇO SEM A BOLA BATER NO CHÃO" — a MESMA lei da S23, feita a UMA pancada
    em vez de a um segmento. O servidor larga a bola -> ela QUICA FUNDO -> e só DEPOIS bate.

    ⚠️ ZERO números novos: reusa o QUIQUE_JANELA (3 s) e o QUIQUE_PROF (0,7) da S23.
       Se um dia se afinar a S23, esta anda com ela — de propósito. É a MESMA regra."""
    j0 = f - int(QUIQUE_JANELA * FPS)
    return any(j0 <= q <= f and q in R and prof(R[q][0], R[q][1])[1] >= QUIQUE_PROF
               for q in RES)
...
# 🔴 S42 — A CONFIRMAÇÃO DO FIM  (lei do Vasco, 13 jul)
#    "Das 4 s para confirmar que não há mais nenhuma pancada, confirmas que a pancada
#     SEGUINTE é o SERVIÇO. E quando estiver confirmado que aquela era a ÚLTIMA pancada,
#     apresentas apenas mais UM SEGUNDO para além dessa pancada."
#
#    ⚠️ SEPARA TRÊS COISAS que o SILENCIO=4 fazia sozinho, misturadas:
#         ① agrupar cruzamentos        -> SILENCIO (4 s)   [inalterado]
#         ② JANELA DE VERIFICAÇÃO      -> SILENCIO (4 s)   "vem aí outra pancada?"  [inalterado]
#         ③ MARGEM DE APRESENTAÇÃO     -> M_CONFIRMADO (1 s)  <- É NOVA. Antes era o M_COM_PAN=2.
#       "A janela de DECISÃO não é a margem de APRESENTAÇÃO." — Vasco
#
...
def rallies(CR, PAN, FIM=(), RES=(), R=None, prof=None):
    ...
    for g in grp:
        ...
        # 🔴 S42 — A CONFIRMAÇÃO DO FIM (Vasco). A última pancada só é a ÚLTIMA se a
        #    pancada SEGUINTE for um SERVIÇO. Confirmado -> corte a 1 s (M_CONFIRMADO).
        #    Não confirmado -> o ponto NÃO acabou -> cai na DÚVIDA (S16), com mais margem.
        #    ⚠️ Só ESTICA. Nunca deita fora um candidato (D18).
        confirmado = False
